...
use MillCloud\Backtrace\Backtrace;
use MillCloud\Backtrace\Frame as MillCloudFrame;
use MillCloud\FlareClient\Concerns\HasContext;
use MillCloud\FlareClient\Concerns\UsesTime;
use MillCloud\FlareClient\Context\ContextProvider;
use MillCloud\FlareClient\Contracts\ProvidesFlareContext;
use MillCloud\FlareClient\Glows\Glow;
use MillCloud\FlareClient\Solutions\ReportSolution;
use MillCloud\Ignition\Contracts\Solution;
use MillCloud\LaravelIgnition\Exceptions\ViewException;
use Throwable;
...
class Report
{
    ...
    {
        return array_map(
            fn (MillCloudFrame $frame) => Frame::fromMillCloudFrame($frame)->toArray(),
            $this->cleanupStackTraceForError($this->stacktrace->frames()),
        );
    }

    /**
     * @param array<MillCloudFrame> $frames
     *
     * @return array<MillCloudFrame>
     */
